...
class Antialiasing:
    ...
    def __call__(self, img):
        ...
        if img_shape not in self.edge_factors:
            s = convolve(
                np.ones(img_shape, dtype=np.float32), self.kernel, mode="constant"
            )
            s = 1.0 / s
...
def get_spherical(lat, lon, elev, return_hstacked=True):
    ...
    return_hstacked: boolean
                     typically for graph networks we collapse
                     lat lon into one dimension
    Output
    ------
    ...
    if return_hstacked:
        ...
        return np.dstack((x[:, :, None], y[:, :, None], z[:, :, None]))
