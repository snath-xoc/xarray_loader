...
class BatchDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if weighted_sampler:
            ...
            unique_classes = xr.DataArray(
                data=[
                    np.quantile(y_train, 0.05).flatten()[0],
                    np.quantile(y_train, 0.5).flatten()[0],
                    np.quantile(y_train, 0.75).flatten()[0],
                    np.quantile(y_train, 0.95).flatten()[0],
                ],
                dims=["x"],
            ...
            sample_weights = weight[
                np.searchsorted(unique_classes, rounded_y_train) - 1
            ]

            self.sample_weights = torch.from_numpy(np.asarray(samples_weight))
            self.sampler = torch.utils.data.WeightedRandomSampler(
                self.sample_weights.type("torch.DoubleTensor"), len(sample_weights)
            )
...
class BatchTruth(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if weighted_sampler:
            ...
            rounded_y_train = y_train  # np.round(y_train, decimals=1)
            unique_classes = xr.DataArray(
                data=[
                    np.quantile(y_train, 0.25).flatten()[0],
                    np.quantile(y_train, 0.5).flatten()[0],
                    np.quantile(y_train, 0.75).flatten()[0],
                    np.quantile(y_train, 0.95).flatten()[0],
                ],
            ...
            # logprec(xr.DataArray(data=[10/24,20/24,60/24,80/24],dims=['x'],coords={
            #    'x':np.arange(4),
            # }))
            # np.unique(rounded_y_train)
            # class_sample_count = np.bincount(
            #    np.digitize(rounded_y_train, unique_classes) - 1
            # )
            weight = np.asarray([0.05, 0.05, 0.2, 0.3, 0.4])  # 1.0 / class_sample_count
            print(np.unique(np.searchsorted(unique_classes, rounded_y_train)))
            sample_weights = weight[
                np.searchsorted(unique_classes, rounded_y_train) - 1
            ]
